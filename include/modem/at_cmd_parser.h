 ...
 *                       are stored. Must not be NULL.
 *
 * @param type           Allows specifying type for list elements, except the first.
 *                       If set to AT_PARAM_TYPE_INVALID, the parser will auto-detect type.
 *
 * @retval 0 If the operation was successful.
 *           Otherwise, a (negative) error code is returned.
 * @retval -EAGAIN New notification detected in string re-run the parser
 ...
 * @retval -EINVAL One or more of the supplied parameters are invalid.
 */
int at_parser_params_from_str_with_type(const char *at_params_str, char **next_param_str,
			      struct at_param_list *const list, int type);
