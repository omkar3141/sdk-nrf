...
Estimated lifetime scales proportionally with :kconfig:option:`CONFIG_BT_MESH_RPL_STORE_TIMEOUT` (for example, 10 s instead of 30 s divides lifetime by three when message rate and dirty entries for each flush are unchanged).
...
* :kconfig:option:`CONFIG_BT_MESH_CRPL` = 32 (NLC default):
