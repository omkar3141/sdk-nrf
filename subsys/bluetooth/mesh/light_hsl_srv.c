...
struct bt_mesh_light_hsl_srv_settings_data {
	struct bt_mesh_light_hsl default_params;
	struct bt_mesh_light_hsl_range_set hsl_range;
	struct bt_mesh_lightness_range light_range;
	uint16_t light_last;
	uint16_t saturation_last;
	uint16_t hue_last;
	bool light_is_on;
} __packed;

static int store_state(struct bt_mesh_light_hsl_srv *srv)
{
	if (!IS_ENABLED(CONFIG_BT_SETTINGS)) {
		return 0;
	}

	struct bt_mesh_light_hsl_srv_settings_data data = {
		.default_params.lightness = srv->lightness_srv.default_light,
		.default_params.saturation = srv->sat_srv.satur_default,
		.default_params.hue = srv->hue_srv.hue_default,
		.hsl_range.saturation = srv->sat_srv.range,
		.hsl_range.hue = srv->hue_srv.range,
		.light_range = srv->lightness_srv.range,
		.light_last = srv->lightness_srv.last,
		.saturation_last = srv->sat_srv.satur_last,
		.hue_last = srv->hue_srv.hue_last,
		.light_is_on = atomic_test_bit(&srv->lightness_srv.flags,
...
{
	bt_mesh_model_msg_init(buf, BT_MESH_LIGHT_HSL_DEFAULT_STATUS);
	net_buf_simple_add_le16(buf, light_to_repr(srv->lightness_srv.default_light,
						   ACTUAL));
	net_buf_simple_add_le16(buf, srv->hue_srv.hue_default);
	net_buf_simple_add_le16(buf, srv->sat_srv.satur_default);
}
			...
			struct bt_mesh_msg_ctx *rx_ctx)
{
	...
	BT_MESH_MODEL_BUF_DEFINE(msg, BT_MESH_LIGHT_HSL_DEFAULT_STATUS,
			...
			bool ack)
{
	...
	srv->lightness_srv.default_light = repr_to_light(net_buf_simple_pull_le16(buf), ACTUAL);
	srv->hue_srv.hue_default = net_buf_simple_pull_le16(buf);
	srv->sat_srv.satur_default = net_buf_simple_pull_le16(buf);
	store_state(srv);

	struct bt_mesh_light_hsl set = {
		.lightness = srv->lightness_srv.default_light,
		.hue = srv->hue_srv.hue_default,
		.saturation = srv->sat_srv.satur_default,
	};

	if (srv->handlers->default_update) {
		srv->handlers->default_update(srv, ctx, &set);
	}

	(void)bt_mesh_light_hsl_default_pub(srv, NULL);
...
static void range_encode_status(struct net_buf_simple *buf,
				...
				enum bt_mesh_model_status status)
{
	bt_mesh_model_msg_init(buf, BT_MESH_LIGHT_HSL_RANGE_STATUS);
	net_buf_simple_add_u8(buf, status);
	net_buf_simple_add_le16(buf, srv->hue_srv.range.min);
	net_buf_simple_add_le16(buf, srv->hue_srv.range.max);
	net_buf_simple_add_le16(buf, srv->sat_srv.range.min);
	net_buf_simple_add_le16(buf, srv->sat_srv.range.max);
}
			   ...
			   enum bt_mesh_model_status status)
{
	...
	BT_MESH_MODEL_BUF_DEFINE(msg, BT_MESH_LIGHT_HSL_RANGE_STATUS,
				 BT_MESH_LIGHT_HSL_MSG_LEN_RANGE_STATUS);
	range_encode_status(&msg, srv, status);
	(void)bt_mesh_model_send(model, rx_ctx, &msg, NULL, NULL);
}

static void range_set(struct bt_mesh_model *model,
			   struct bt_mesh_msg_ctx *ctx,
			   struct net_buf_simple *buf, bool ack)
{
	...
	set.hue.min = net_buf_simple_pull_le16(buf);
	set.hue.max = net_buf_simple_pull_le16(buf);
	set.saturation.min = net_buf_simple_pull_le16(buf);
	set.saturation.max = net_buf_simple_pull_le16(buf);

	if ((set.hue.min > set.hue.max) ||
	    (set.saturation.min > set.saturation.max)) {
		status_code = BT_MESH_MODEL_STATUS_INVALID;
		goto respond;
	}

	srv->hue_srv.range = set.hue;
	srv->sat_srv.range = set.saturation;
	store_state(srv);
	status = BT_MESH_MODEL_SUCCESS;

	if (srv->handlers->range_update) {
		srv->handlers->range_update(srv, ctx, &set);
	}

	(void)bt_mesh_light_hsl_srv_range_pub(srv, NULL, status);

respond:
	if (ack) {
		range_rsp(model, ctx, status);
	}
}
...
const struct bt_mesh_model_op _bt_mesh_light_hsl_srv_op[] = {
	{ BT_MESH_LIGHT_HSL_GET, BT_MESH_LIGHT_HSL_MSG_LEN_GET,
	  hsl_get_handle },
	{ BT_MESH_LIGHT_HSL_SET, BT_MESH_LIGHT_HSL_MSG_MINLEN_SET,
	  hsl_set_handle },
	{ BT_MESH_LIGHT_HSL_SET_UNACK, BT_MESH_LIGHT_HSL_MSG_MINLEN_SET,
	  hsl_set_unack_handle },
	{ BT_MESH_LIGHT_HSL_TARGET_GET, BT_MESH_LIGHT_HSL_MSG_LEN_GET,
	  hsl_target_get_handle },
	{ BT_MESH_LIGHT_HSL_DEFAULT_GET, BT_MESH_LIGHT_HSL_MSG_LEN_GET,
	  default_get_handle },
	{ BT_MESH_LIGHT_HSL_RANGE_GET, BT_MESH_LIGHT_HSL_MSG_LEN_GET,
	  range_get_handle },
	BT_MESH_MODEL_OP_END,
};

const struct bt_mesh_model_op _bt_mesh_light_hsl_setup_srv_op[] = {
	{ BT_MESH_LIGHT_HSL_DEFAULT_SET,
	  BT_MESH_LIGHT_HSL_MSG_LEN_DEFAULT, default_set_handle },
	{ BT_MESH_LIGHT_HSL_DEFAULT_SET_UNACK,
	  BT_MESH_LIGHT_HSL_MSG_LEN_DEFAULT, default_set_unack_handle },
	{ BT_MESH_LIGHT_HSL_RANGE_SET,
	  BT_MESH_LIGHT_HSL_MSG_LEN_RANGE_SET, range_set_handle },
	{ BT_MESH_LIGHT_HSL_RANGE_SET_UNACK,
	  BT_MESH_LIGHT_HSL_MSG_LEN_RANGE_SET, range_set_unack_handle },
	BT_MESH_MODEL_OP_END,
};
...
static int bt_mesh_light_hsl_srv_settings_set(struct bt_mesh_model *model,
					 ...
					 settings_read_cb read_cb, void *cb_arg)
{
	...
	srv->lightness_srv.default_light = data.default_params.lightness;
	srv->sat_srv.satur_default = data.default_params.saturation;
	srv->hue_srv.hue_default = data.default_params.hue;
	srv->sat_srv.range = data.hsl_range.saturation;
	srv->hue_srv.range = data.hsl_range.hue;
	srv->lightness_srv.range = data.light_range;
	srv->lightness_srv.last = data.light_last;
	srv->sat_srv.satur_last = data.saturation_last;
	srv->hue_srv.hue_last = data.hue_last;
	atomic_set_bit_to(&srv->lightness_srv.flags, LIGHTNESS_SRV_FLAG_IS_ON,
			  data.light_is_on);
...
static int bt_mesh_light_hsl_srv_start(struct bt_mesh_model *mod)
{
	...
	if (!srv->sat_srv.model ||
	    (srv->model->elem_idx > srv->sat_srv.model->elem_idx)) {
		BT_ERR("Light HSL srv[%d]: Sat. srv not properly initialized",
		       srv->model->elem_idx);
		return -EINVAL;
	}

	if (!srv->hue_srv.model ||
...
const struct bt_mesh_model_cb _bt_mesh_light_hsl_srv_cb = {
	...
	.settings_set = bt_mesh_light_hsl_srv_settings_set,
};
...
int32_t
bt_mesh_light_hsl_srv_target_pub(struct bt_mesh_light_hsl_srv *srv,
				 struct bt_mesh_msg_ctx *ctx,
				 struct bt_mesh_light_hsl_status *status)
{
}
...
int32_t
bt_mesh_light_hsl_srv_range_pub(struct bt_mesh_light_hsl_srv *srv,
				struct bt_mesh_msg_ctx *ctx,
