...
static void status_decode(struct bt_mesh_light_hsl_cli *cli,
			  ...
			  struct bt_mesh_hsl_status *status)
{
	if (buf->len != BT_MESH_HSL_MSG_MINLEN_STATUS &&
	    buf->len != BT_MESH_HSL_MSG_MAXLEN_STATUS) {
		return;
	}
...
{
	if (buf->len != BT_MESH_HSL_MSG_MINLEN_HUE_STATUS &&
	    buf->len != BT_MESH_HSL_MSG_MAXLEN_HUE_STATUS) {
		return;
	}
				  ...
				  struct net_buf_simple *buf)
{
	...
	status_decode(cli, ctx, buf, BT_MESH_HSL_STATUS, &status);
				  ...
				  struct net_buf_simple *buf)
{
	...
	status_decode(cli, ctx, buf, BT_MESH_HSL_TARGET_STATUS, &status);
...
static void default_status_handle(struct bt_mesh_model *model,
				  ...
				  struct net_buf_simple *buf)
{
	if (buf->len != BT_MESH_HSL_DEFAULT_STATUS) {
		return;
	}

	struct bt_mesh_light_hsl_cli *cli = model->user_data;
	struct bt_mesh_hsl status;

	status.lightness = net_buf_simple_pull_le16(buf);
	status.hue = net_buf_simple_pull_le16(buf);
	status.saturation = net_buf_simple_pull_le16(buf);

	if (model_ack_match(&cli->ack_ctx, BT_MESH_HSL_DEFAULT_STATUS,
			    ctx)) {
		struct bt_mesh_hsl *rsp =
			(struct bt_mesh_hsl *)cli->ack_ctx.user_data;
		*rsp = status;
		model_ack_rx(&cli->ack_ctx);
	}
...
static void range_status_handle(struct bt_mesh_model *model,
				  ...
				  struct net_buf_simple *buf)
{
	if (buf->len != BT_MESH_HSL_MSG_LEN_RANGE_STATUS) {
		return;
	}

	struct bt_mesh_light_hsl_cli *cli = model->user_data;
	struct bt_mesh_hsl_range_status status;
	...
	if (model_ack_match(&cli->ack_ctx, BT_MESH_HSL_MSG_LEN_RANGE_STATUS,
			    ctx)) {
				  ...
				  struct net_buf_simple *buf)
{
	...
	hue_status_decode(cli, ctx, buf, BT_MESH_HSL_HUE_STATUS, &status);
				  ...
				  struct net_buf_simple *buf)
{
	...
	hue_status_decode(cli, ctx, buf, BT_MESH_HSL_SATURATION_STATUS, &status);
...
const struct bt_mesh_model_op _bt_mesh_light_hsl_cli_op[] = {
	{ BT_MESH_HSL_STATUS, BT_MESH_HSL_MSG_MINLEN_STATUS,
	  hsl_status_handle },
	{ BT_MESH_HSL_TARGET_STATUS, BT_MESH_HSL_MSG_MINLEN_STATUS,
	  hsl_target_status_handle },
	{ BT_MESH_HSL_DEFAULT_STATUS, BT_MESH_HSL_MSG_LEN_DEFAULT,
	  default_status_handle },
	{ BT_MESH_HSL_RANGE_STATUS, BT_MESH_HSL_MSG_LEN_RANGE_STATUS,
	  range_status_handle },
	{ BT_MESH_HSL_HUE_STATUS, BT_MESH_HSL_MSG_MINLEN_HUE_STATUS,
	  hue_status_handle },
	{ BT_MESH_HSL_SATURATION_STATUS, BT_MESH_HSL_MSG_MINLEN_HUE_STATUS,
...
static int get_msg(struct bt_mesh_light_hsl_cli *cli,
		   ...
		   uint16_t ret_opcode)
{
	BT_MESH_MODEL_BUF_DEFINE(msg, opcode, BT_MESH_HSL_MSG_LEN_GET);
...
int bt_mesh_light_hsl_get(struct bt_mesh_light_hsl_cli *cli,
			  ...
			  struct bt_mesh_hsl_status *rsp)
{
	return get_msg(cli, ctx, rsp, BT_MESH_HSL_GET,
		       BT_MESH_HSL_STATUS);
}
...
int bt_mesh_light_hsl_set(struct bt_mesh_light_hsl_cli *cli,
			  ...
			  struct bt_mesh_hsl_status *rsp)
{
	BT_MESH_MODEL_BUF_DEFINE(msg, BT_MESH_HSL_SET,
				 BT_MESH_HSL_MSG_MAXLEN_SET);
	bt_mesh_model_msg_init(&msg, BT_MESH_HSL_SET);
...
int bt_mesh_light_hsl_set_unack(struct bt_mesh_light_hsl_cli *cli,
				...
				const struct bt_mesh_hsl_set *set)
{
	BT_MESH_MODEL_BUF_DEFINE(msg, BT_MESH_HSL_SET_UNACK,
				 BT_MESH_HSL_MSG_MAXLEN_SET);
	bt_mesh_model_msg_init(&msg, BT_MESH_HSL_SET_UNACK);
	net_buf_simple_add_le16(&msg, set->params.lightness);
	net_buf_simple_add_le16(&msg, set->params.hue);
	net_buf_simple_add_le16(&msg, set->params.saturation);
	net_buf_simple_add_u8(&msg, cli->tid++);
...
int bt_mesh_light_hsl_target_get(struct bt_mesh_light_hsl_cli *cli,
				 ...
				 struct bt_mesh_hsl_status *rsp)
{
	return get_msg(cli, ctx, rsp, BT_MESH_HSL_TARGET_GET,
		       BT_MESH_HSL_TARGET_STATUS);
}
...
int bt_mesh_light_hsl_default_get(struct bt_mesh_light_hsl_cli *cli,
				  ...
				  struct bt_mesh_hsl *rsp)
{
	return get_msg(cli, ctx, rsp, BT_MESH_HSL_DEFAULT_GET,
		       BT_MESH_HSL_DEFAULT_STATUS);
}
...
int bt_mesh_light_hsl_default_set(struct bt_mesh_light_hsl_cli *cli,
				  ...
				  struct bt_mesh_hsl *rsp)
{
	BT_MESH_MODEL_BUF_DEFINE(msg, BT_MESH_HSL_DEFAULT_SET,
				 BT_MESH_HSL_MSG_LEN_DEFAULT);
	bt_mesh_model_msg_init(&msg, BT_MESH_HSL_DEFAULT_SET);
	...
	return model_ackd_send(cli->model, ctx, &msg,
			       rsp ? &cli->ack_ctx : NULL,
			       BT_MESH_HSL_DEFAULT_STATUS, rsp);
}
...
int bt_mesh_light_hsl_default_set_unack(struct bt_mesh_light_hsl_cli *cli,
					...
					const struct bt_mesh_hsl *set)
{
	BT_MESH_MODEL_BUF_DEFINE(msg, BT_MESH_HSL_DEFAULT_SET_UNACK,
				 BT_MESH_HSL_MSG_LEN_DEFAULT);
	bt_mesh_model_msg_init(&msg, BT_MESH_HSL_DEFAULT_SET_UNACK);
	net_buf_simple_add_le16(&msg, set->lightness);
	net_buf_simple_add_le16(&msg, set->hue);
	net_buf_simple_add_le16(&msg, set->saturation);
...
int bt_mesh_light_hsl_range_get(struct bt_mesh_light_hsl_cli *cli,
				...
				struct bt_mesh_hsl_range_status *rsp)
{
	return get_msg(cli, ctx, rsp, BT_MESH_HSL_RANGE_GET,
		       BT_MESH_HSL_RANGE_STATUS);
}
...
int bt_mesh_light_hsl_range_set(struct bt_mesh_light_hsl_cli *cli,
				...
				struct bt_mesh_hsl_range_status *rsp)
{
	BT_MESH_MODEL_BUF_DEFINE(msg, BT_MESH_HSL_RANGE_SET,
				 BT_MESH_HSL_MSG_LEN_RANGE_SET);
	bt_mesh_model_msg_init(&msg, BT_MESH_HSL_RANGE_SET);
...
int bt_mesh_light_hsl_range_set_unack(struct bt_mesh_light_hsl_cli *cli,
				      ...
				      const struct bt_mesh_hsl_range_set *set)
{
	BT_MESH_MODEL_BUF_DEFINE(msg, BT_MESH_HSL_RANGE_SET_UNACK,
				 BT_MESH_HSL_MSG_LEN_RANGE_SET);
	bt_mesh_model_msg_init(&msg, BT_MESH_HSL_RANGE_SET_UNACK);
	net_buf_simple_add_le16(&msg, set->hue.min);
	net_buf_simple_add_le16(&msg, set->hue.max);
	net_buf_simple_add_le16(&msg, set->saturation.min);
	net_buf_simple_add_le16(&msg, set->saturation.max);
...
int bt_mesh_light_hsl_hue_get(struct bt_mesh_light_hsl_cli *cli,
			      ...
			      struct bt_mesh_hsl_hue_status *rsp)
{
	return get_msg(cli, ctx, rsp, BT_MESH_HSL_HUE_GET,
		       BT_MESH_HSL_HUE_STATUS);
}
...
int bt_mesh_light_hsl_hue_set(struct bt_mesh_light_hsl_cli *cli,
			      ...
			      struct bt_mesh_hsl_hue_status *rsp)
{
	BT_MESH_MODEL_BUF_DEFINE(msg, BT_MESH_HSL_HUE_SET,
				 BT_MESH_HSL_MSG_MAXLEN_HUE);
	bt_mesh_model_msg_init(&msg, BT_MESH_HSL_HUE_SET);
...
int bt_mesh_light_hsl_hue_set_unack(struct bt_mesh_light_hsl_cli *cli,
				    ...
				    const struct bt_mesh_hsl_hue_set *set)
{
	BT_MESH_MODEL_BUF_DEFINE(msg, BT_MESH_HSL_HUE_SET_UNACK,
				 BT_MESH_HSL_MSG_MAXLEN_HUE);
	bt_mesh_model_msg_init(&msg, BT_MESH_HSL_HUE_SET_UNACK);
	net_buf_simple_add_le16(&msg, set->level);
	net_buf_simple_add_u8(&msg, cli->tid++);
	if (set->transition) {
		model_transition_buf_add(&msg, set->transition);
...
int bt_mesh_light_hsl_saturation_get(struct bt_mesh_light_hsl_cli *cli,
				     ...
				     struct bt_mesh_hsl_hue_status *rsp)
{
	return get_msg(cli, ctx, rsp, BT_MESH_HSL_SATURATION_GET,
		       BT_MESH_HSL_SATURATION_STATUS);
}
...
int bt_mesh_light_hsl_saturation_set(struct bt_mesh_light_hsl_cli *cli,
				     ...
				     struct bt_mesh_hsl_hue_status *rsp)
{
	BT_MESH_MODEL_BUF_DEFINE(msg, BT_MESH_HSL_SATURATION_SET,
				 BT_MESH_HSL_MSG_MAXLEN_HUE);
	bt_mesh_model_msg_init(&msg, BT_MESH_HSL_SATURATION_SET);
	net_buf_simple_add_le16(&msg, set->level);
	net_buf_simple_add_u8(&msg, cli->tid++);
	if (set->transition) {
		model_transition_buf_add(&msg, set->transition);
	...
	return model_ackd_send(cli->model, ctx, &msg,
			       rsp ? &cli->ack_ctx : NULL,
			       BT_MESH_HSL_SATURATION_STATUS, rsp);
}
...
int bt_mesh_light_hsl_saturation_set_unack(struct bt_mesh_light_hsl_cli *cli,
					   ...
					   const struct bt_mesh_hsl_hue_set *set)
{
	BT_MESH_MODEL_BUF_DEFINE(msg, BT_MESH_HSL_SATURATION_SET_UNACK,
				 BT_MESH_HSL_MSG_MAXLEN_HUE);
	bt_mesh_model_msg_init(&msg, BT_MESH_HSL_SATURATION_SET_UNACK);
	net_buf_simple_add_le16(&msg, set->level);
	net_buf_simple_add_u8(&msg, cli->tid++);
	if (set->transition) {
		model_transition_buf_add(&msg, set->transition);
