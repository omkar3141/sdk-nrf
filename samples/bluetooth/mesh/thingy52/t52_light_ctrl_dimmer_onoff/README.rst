.. _t52_bluetooth_mesh_light_lc:

Bluetooth mesh: Light fixture (Thingy52)
#############################
...
This is a fork of sample :ref:`bluetooth_mesh_light_lc`. This fork supports Thingy52 and additionally adds Dimming Control NLC profile to the sample.
...
The client models are used to control other corresponding server models for the same sample running on a different device.
...
* The OnOff Client model and Level Client model on first element is used to control the peer models of the same sample running on the same or different device. These models are interfaced with single button on the Thingy52.
...
.. note::
   :ref:`zephyr:thingy52_nrf52832` supports only one RGB LED.
